    ...
    def apply_fill(self, fill: Fill) -> None:
        ...
        if fill.side == OrderSide.BUY:
            ...
            if position.quantity < 0:
                ...
                pnl = (position.avg_price - fill.price) * close_quantity - fill.commission
            ...
            if position.quantity > 0:
                ...
                pnl = (fill.price - position.avg_price) * close_quantity - fill.commission
                position.realized_pnl += pnl
                position.quantity -= close_quantity
                ...
                total_proceeds = abs(position.avg_price * position.quantity) + fill.quantity * fill.price
                position.quantity -= fill.quantity
                position.avg_price = abs(total_proceeds / position.quantity) if position.quantity < 0 else 0
