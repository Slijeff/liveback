...
def main():
    ...
    # Set up event bus for event routing
    event_bus = EventBus()
    ...
    engine = BacktestEngine(
        ...
        strategy=strategy,
        portfolio=portfolio,
        risk_manager=None,  # Optional: can add risk manager later
        event_bus=event_bus
    )
    ...
    for symbol, position in portfolio.positions.items():
        if position.quantity != 0:
            print(f"  {symbol}: {position.quantity:.2f} shares @ ${position.avg_price:.2f}")
